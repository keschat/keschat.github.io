(function($) {
	 ...
	 */ $.fn.placeholder = function() {
        ...
        $this.find("input[type=password]").each(function() {
            var i1 = $(this);
            var x1 = $($("<div>").append(i1.clone()).remove().html().replace(/type="password"/i, 'type="text"').replace(/type=password/i, "type=text"));
            if (i1.attr("id") != "") x1.attr("id", i1.attr("id") + "-polyfill-field");
            if (i1.attr("name") != "") x1.attr("name", i1.attr("name") + "-polyfill-field");
            x1.addClass("polyfill-placeholder").val(x1.attr("placeholder")).insertAfter(i1);
            if (i1.val() == "") i1.hide();
            else x1.hide();
            i1.on("blur", function(event) {
                event.preventDefault();
                var x = i1.parent().find("input[name=" + i1.attr("name") + "-polyfill-field]");
                if (i1.val() == "") {
                    i1.hide();
                    x.show();
                }
            });
            x1.on("focus", function(event) {
                event.preventDefault();
                var i = x1.parent().find("input[name=" + x1.attr("name").replace("-polyfill-field", "") + "]");
                x1.hide();
                i.show().focus();
...
//# sourceMappingURL=index.fac0ddec.js.map
